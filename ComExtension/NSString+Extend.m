...
//    fmt.dateFormat = @"MM月dd日";
    ...
    resultStr = [NSString stringWithFormat:@"%@", send];
    return timeStr;
}
...
+ (NSString *)dealNum:(NSNumber *)num {
    ...
    return [NSString stringWithFormat:@"%.f %@", floatValue, unitStr];
}

// YES表示开始时间小于结束时间  NO表示开始时间大于结束时间
+ (BOOL)startTimeAndEndTime: (NSString *)startTimeStr endTime:(NSString *)endStr {
    
    NSString *startTime = [startTimeStr stringByReplacingOccurrencesOfString:@"-" withString:@""];
    NSString *endTime = [endStr stringByReplacingOccurrencesOfString:@"-" withString:@""];
